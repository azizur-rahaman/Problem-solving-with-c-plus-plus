...
void solve(double meal_cost, int tip_percent, int tax_percent) {
    ...
    double total_meal_cost = meal_cost+totalTip+totalTax;
    int totalInInt = total_meal_cost;
    
    if(total_meal_cost>((double)totalInInt+0.5)){
        cout<<(int)total_meal_cost+1<<endl;
    }else {
        cout<<(int)total_meal_cost<<endl;
    }
}
