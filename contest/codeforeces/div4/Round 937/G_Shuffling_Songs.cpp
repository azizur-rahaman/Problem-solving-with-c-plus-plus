...
void solve() {
    ...
    for(int i = 0; i<n; i++){
        string t1,t2;
        cin>>t1>>t2;
        vp[i] = make_pair(t1,t2);
    }

    set<string> settG;
    set<string> settW;
    settG.insert(vp[0].first);
    settW.insert(vp[0].second);

    int cnt = 1;

    for (int i = 1; i < n; i++)
    {
        if(settG.find(vp[i].first) != settG.end() || settW.find(vp[i].second) != settW.end()){
            cnt++;
            settG.insert(vp[i].first);
            settW.insert(vp[i].second);
        }
    }
